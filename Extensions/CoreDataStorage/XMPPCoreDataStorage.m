...
- (NSString *)managedObjectModelName {
  ...
  //
  // This method is queried to get the name of the ManagedObjectModel within the
  // app bundle.
  // It should return the name of the appropriate file (*.xdatamodel / *.mom /
  // *.momd) sans file extension.
  //
  // The default implementation returns the name of the subclass, stripping any
  // suffix of "CoreDataStorage".
  // E.g., if your subclass was named "XMPPExtensionCoreDataStorage", then this
  // method would return "XMPPExtension".
  //
...
- (NSString *)defaultDatabaseFileName {
  // Override me, if needed, to provide customized behavior.
  //
  // This method is queried if the initWithDatabaseFileName:storeOptions: method
  // is invoked with a nil parameter for databaseFileName.
  //
...
- (NSDictionary *)defaultStoreOptions {
  ...
  //
  // This method is queried if the initWithDatabaseFileName:storeOptions: method
  // is invoked with a nil parameter for defaultStoreOptions.
...
- (void)willCreatePersistentStoreWithPath:(NSString *)storePath
                                  options:(NSDictionary *)theStoreOptions {
  // Override me, if needed, to provide customized behavior.
  //
  // If you are using a database file with pure non-persistent data (e.g. for
  // memory optimization purposes on iOS),
  // you may want to delete the database file if it already exists on disk.
  //
  // If this instance was created via initWithDatabaseFilename, then the
  // storePath parameter will be non-nil.
  // If this instance was created via initWithInMemoryStore, then the storePath
  // parameter will be nil.
}

- (BOOL)addPersistentStoreWithPath:(NSString *)storePath
                           options:(NSDictionary *)theStoreOptions
...
- (void)didNotAddPersistentStoreWithPath:(NSString *)storePath
                                 options:(NSDictionary *)theStoreOptions
                                   error:(NSError *)error {
// Override me, if needed, to provide customized behavior.
//
// For example, if you are using the database for non-persistent data and the
// model changes,
// you may want to delete the database file if it already exists on disk.
//
// E.g:
...
- (void)didCreateManagedObjectContext {
  ...
  // For example, you may want to perform cleanup of any non-persistent data
  // before you start using the database.
  //
  // This method is invoked on the storageQueue.
}

- (void)willSaveManagedObjectContext {
  // Override me if you need to do anything special just before changes are
  // saved to disk.
  //
  // This method is invoked on the storageQueue.
}
...
- (void)didSaveManagedObjectContext {
  // Override me if you need to do anything special after changes have been
  // saved to disk.
  //
  // This method is invoked on the storageQueue.
}
...
- (void)mainThreadManagedObjectContextDidMergeChanges {
  // Override me if you want to do anything special when changes get propogated
  // to the main thread.
  //
...
- (void)commonInit {
  ...
  dispatch_queue_set_specific(storageQueue, storageQueueTag, storageQueueTag,
                              NULL);
...
- (id)initWithDatabaseFilename:(NSString *)aDatabaseFileName
                  storeOptions:(NSDictionary *)theStoreOptions {
  if ((self = [super init])) {
    ...
    [self commonInit];
    NSAssert(storageQueue != NULL,
             @"Subclass forgot to invoke [super commonInit]");
  }
  return self;
}
...
- (BOOL)configureWithParent:(id)aParent queue:(dispatch_queue_t)queue {
  // This is the standard configure method used by xmpp extensions to configure
  // a storage class.
  //
  // Feel free to override this method if needed,
  // and just invoke super at some point to make sure everything is kosher at
  // this level as well.
  ...
  if (queue == storageQueue) {
    // This class is designed to be run on a separate dispatch queue from its
    // parent.
    // This allows us to optimize the database save operations by buffering
    // them,
...
//
// The motivation behind this is the fact that to query the xmppStream for its
// myJID
// requires going through the xmppStream's internal dispatch queue. (A
// dispatch_sync).
// It's not necessarily that this is an expensive operation,
// but the storage classes sometimes require this information for just about
// every operation they perform.
// For a variable that changes infrequently, caching the value can reduce some
// overhead.
...
- (void)didChangeCachedMyJID:(XMPPJID *)cachedMyJID
               forXMPPStream:(XMPPStream *)stream {
  // Override me if you'd like to do anything special when this happens.
  //
  // For example, if your custom storage class prefetches data related to the
  // current user.
}
...
- (NSString *)persistentStoreDirectory {
  ...
  // Previously the Peristent Story Directory was based on the Bundle Display
  // Name but this can be Localized
  // If Peristent Story Directory already exists we will use that
  //    NSString *bundleDisplayName = [[NSBundle mainBundle]
  //    objectForInfoDictionaryKey:@"CFBundleDisplayName"];
  //    NSString *legacyPersistentStoreDirectory  = [basePath
  //    stringByAppendingPathComponent:bundleDisplayName];
  //    if (bundleDisplayName && [fileManager
  //    fileExistsAtPath:legacyPersistentStoreDirectory]) {
  ...
  NSString *persistentStoreDirectory =
      [basePath stringByAppendingPathComponent:bundleIdentifier];
  if (![fileManager fileExistsAtPath:persistentStoreDirectory]) {
    [fileManager createDirectoryAtPath:persistentStoreDirectory
...
- (NSManagedObjectModel *)managedObjectModel {
  ...
  dispatch_block_t block = ^{
    @autoreleasepool {
      ...
      NSString *momPath =
          [[self managedObjectModelBundle] pathForResource:momName
                                                    ofType:@"mom"];
      if (momPath == nil) {
        // The model may be versioned or created with Xcode 4, try momd as an
        // extension.
        momPath = [[self managedObjectModelBundle] pathForResource:momName
                                                            ofType:@"momd"];
      }

      if (momPath) {
        // If path is nil, then NSURL or NSManagedObjectModel will throw an
        // exception

        NSURL *momUrl = [NSURL fileURLWithPath:momPath];

        managedObjectModel =
            [[[NSManagedObjectModel alloc] initWithContentsOfURL:momUrl] copy];
      } else {
        XMPPLogWarn(@"%@: Couldn't find managedObjectModel file - %@",
                    [self class], momName);
      }

      if ([NSAttributeDescription
        ...
        if (autoAllowExternalBinaryDataStorage) {
          ...
          for (NSEntityDescription *entity in entities) {
            ...
            [attributesByName enumerateKeysAndObjectsUsingBlock:^(
  ...
  dispatch_block_t block = ^{
    @autoreleasepool {
      ...
      if (databaseFileName) {
        ...
        NSString *storePath =
            [docsPath stringByAppendingPathComponent:databaseFileName];
        if (storePath) {
          ...
          BOOL didAddPersistentStore =
              [self addPersistentStoreWithPath:storePath
                                       options:storeOptions
                                         error:&error];
          ...
          if (!didAddPersistentStore) {
            [self didNotAddPersistentStoreWithPath:storePath
                                           options:storeOptions
                                             error:error];
          }
        } else {
          XMPPLogWarn(@"%@: Error creating persistentStoreCoordinator - Nil "
        ...
        NSError *error = nil;
        if (![self addPersistentStoreWithPath:nil
                                      options:storeOptions
                                        error:&error]) {
          [self didNotAddPersistentStoreWithPath:nil
                                         options:storeOptions
                                           error:error];
        }
      }
...
- (NSManagedObjectContext *)managedObjectContext {
  ...
  // Therefore it is VERY VERY BAD to use our private managedObjectContext
  // outside our private storageQueue.
  //
  // You should NOT remove the assert statement below!
  // You should NOT give external classes access to the storageQueue! (Excluding
  // subclasses obviously.)
  //
  // When you want a managedObjectContext of your own (again, excluding
  // subclasses),
  ...
  //
  NSAssert(dispatch_get_specific(storageQueueTag),
  ...
  if (coordinator) {
    ...
    if ([NSManagedObjectContext
            instancesRespondToSelector:@selector(initWithConcurrencyType:)])
...
- (NSManagedObjectContext *)mainThreadManagedObjectContext {
  // NSManagedObjectContext is NOT thread-safe.
  // Therefore it is VERY VERY BAD to use this managedObjectContext outside the
  // main thread.
  //
  // You should NOT remove the assert statement below!
  //
  ...
  NSPersistentStoreCoordinator *coordinator = [self persistentStoreCoordinator];
  if (coordinator) {
    XMPPLogVerbose(@"%@: Creating mainThreadManagedObjectContext",
                   [self class]);

    if ([NSManagedObjectContext
            instancesRespondToSelector:@selector(initWithConcurrencyType:)])
      mainThreadManagedObjectContext = [[NSManagedObjectContext alloc]
    ...
    // Todo: If we knew that our private managedObjectContext was going to be
    // the only one writing to the database,
    // then a small optimization would be to use it as the object when
    // registering above.
  }
...
- (void)managedObjectContextDidSave:(NSNotification *)notification {
  NSManagedObjectContext *sender =
      (NSManagedObjectContext *)[notification object];

  if ((sender != mainThreadManagedObjectContext) &&
      (sender.persistentStoreCoordinator ==
       mainThreadManagedObjectContext.persistentStoreCoordinator)) {
    XMPPLogVerbose(
        @"%@: %@ - Merging changes into mainThreadManagedObjectContext",
...
- (void)save {
  // I'm fairly confident that the implementation of [NSManagedObjectContext
  // save:]
  // internally checks to see if it has anything to save before it actually does
  // anthing.
  // So there's no need for us to do it here, especially since this method is
  // usually
  ...
  if ([[self managedObjectContext] save:&error]) {
    ...
    XMPPLogWarn(@"%@: Error saving - %@ %@", [self class], error,
                [error userInfo]);
...
- (void)maybeSave:(int32_t)currentPendingRequests {
  ...
  if ([[self managedObjectContext] hasChanges]) {
    if (currentPendingRequests == 0) {
      XMPPLogVerbose(@"%@: Triggering save (pendingRequests=%i)", [self class],
      ...
      if (unsavedCount >= saveThreshold) {
        XMPPLogVerbose(@"%@: Triggering save (unsavedCount=%lu)", [self class],
                       (unsigned long)unsavedCount);
...
- (void)maybeSave {
  // Convenience method in the very rare case that a subclass would need to
  // invoke maybeSave manually.
...
- (void)executeBlock:(dispatch_block_t)block {
  ...
  //
  // If you remove the assert statement below, you are destroying the sole
  ...
  NSAssert(!dispatch_get_specific(storageQueueTag),
           @"Invoked on incorrect queue");
  //
  // For a full discussion of this method, please see
  // XMPPCoreDataStorageProtocol.h
  //
  // dispatch_Sync
  //          ^
  ...
  dispatch_sync(storageQueue, ^{
    @autoreleasepool {
      ...
      // Since this is a synchronous request, we want to return as quickly as
      // possible.
